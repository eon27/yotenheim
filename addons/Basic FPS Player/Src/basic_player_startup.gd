...
var addedHead = false
var step_played : bool = true

@export_category("Audio")
@onready var audioWalking : AudioStreamPlayer3D = $audio/walkingAudio
@onready var audioRunning : AudioStreamPlayer3D = $audio/runningAudio
@onready var audioInput : AudioStreamPlayer = $audio/Input
var audioIndex : int
var audioEffect : AudioEffectCapture
var audioPlayback : AudioStreamGeneratorPlayback
@export var audioOutput : NodePath
@export var audioThreashold = 0.005
var audioRecieveBuffer := PackedFloat32Array()
...
@export var HEAD_BOB_AMPLITUDE := 0.03
...
func motion_sounds():
	if velocity && is_on_floor():
		# Using cos instead of sin because we check when head is going up and down
		# Head is moving up and down with sin and the derivative of sin is cos
		# so we check if it is increasing or decreasing.
		if cos(tick * HEAD_BOB_FREQUENCY) < 0:
			if not step_played:
				if Input.is_action_pressed("shift"):
					$audio/runningAudio.play()
					step_played = true
				else:
					$audio/walkingAudio.play()
					step_played = true
		elif cos(tick * HEAD_BOB_FREQUENCY) > 0:
			step_played = false
	else:
		step_played = false
...
#func setupAudio(id):
	...
	#audioInput.stream = AudioStreamMicrophone.new()
	#audioInput.play()
	#audioIndex = AudioServer.get_bus_index("microphone")
	#audioEffect = AudioServer.get_bus_effect(audioIndex, 0)
	#
	#audioPlayback = get_node(audioOutput).get_stream_playback()
#
#func processMic():
	#if is_multiplayer_authority():
		#var sterioData : PackedVector2Array = audioEffect.get_buffer(audioEffect.get_frames_available())
			...
			#for i in range(sterioData.size()):
				...
				#data[i] = value
			#if maxAmplitude < audioThreashold:
				#return
			#
			##sendAudio.rpc(data)
			#print(data)
#
#func processAudio():
	#if audioRecieveBuffer.size() <= 0:
		#return
	#for i in range(min(audioPlayback.get_frames_available(), audioRecieveBuffer.size())):
		#audioPlayback.push_frame(Vector2(audioRecieveBuffer[0], audioRecieveBuffer[0]))
		#audioRecieveBuffer.remove_at(0)
#
#@rpc("any_peer", "call_remote", "unreliable_ordered")
#func sendAudio(data: PackedFloat32Array, id):
	#audioRecieveBuffer.append_array(data)
	#
	#
